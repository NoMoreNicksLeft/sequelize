    ...
    attributeToSQL: function(attribute, options) {
      ...
      if (attribute.validate) {
        ...
        if (attribute.validate.is) {
          if (attribute.validate.is.msg) {
            template += ' CONSTRAINT "<%= constraintName %>"';
            replacements.constraintName = attribute.validate.is.msg;
            // If we have a msg, that means the validator is in .args[0]
            validator = attribute.validate.is.args;
          }
          else {
            validator = attribute.validate.is;
          }
          template += ' CHECK ("<%= columnName %>" ~<%= modifier %> <%= regex %>)';
          replacements.columnName = attribute.fieldName;
          // There are two possibilities. The regex string is in a js array, or exists as a native regex. Ugh.
          // None of the other standard regex modifiers make sense here except 'i', it's not as if multiline or global
          // could do anything.
          if (validator instanceof RegExp) {
            var match = /^\/(.+?)\/(.*)$/.exec(validator.toString());
            replacements.regex = this.escape(match[1], attribute);
            if (match[2].indexOf('i') > -1) { replacements.modifier = '*'; }
          }
          else {
            replacements.regex = this.escape(validator[0], attribute);
            if (validator[1].indexOf('i') > -1) { replacements.modifier = '*' }
          }
        }

        // validate "not"
        if (attribute.validate.not) {
          if (attribute.validate.not.msg) {
            template += ' CONSTRAINT "<%= constraintName %>"';
            replacements.constraintName = attribute.validate.not.msg;
            validator = attribute.validate.not.args;
          }
          else {
            validator = attribute.validate.not;
          }
          template += ' CHECK ("<%= columnName %>" !~<%= modifier %> <%= regex %>)';
          replacements.columnName = attribute.fieldName;
          // There are two possibilities. The regex string is in a js array, or exists as a native regex. Ugh.
        ...
        if (attribute.validate.isEmail) {
          if (attribute.validate.isEmail.msg) {
            template += ' CONSTRAINT "<%= constraintName %>"';
            replacements.constraintName = attribute.validate.isEmail.msg;
          }
          // Doesn't match Sequelize's test, found at:
          // https://github.com/chriso/validator.js/blob/1.5.0/lib/validators.js#l27
          // Can't match it either, that's using lookaheads and lookbehinds and whatever. 
          template += ' CHECK ("<%= columnName %>" ~ <%= test1 %>';
          template += ' AND "<%= columnName %>" !~ <%= test2 %>';
          template += ' AND "<%= columnName %>" ~ <%= test3 %>)';
          replacements.columnName = attribute.fieldName;
          replacements.test1 = this.escape("^[0-9a-zA-Z!#$%&'*+-/=?^_`{|}~.]{1,64}@([0-9a-z-]+\\.)*[0-9a-z-]+$", attribute);
          replacements.test2 = this.escape("(^\\.|\\.\\.|\\.@)", attribute);
          replacements.test3 = this.escape("@.{1,255}", attribute);
        }
        ...
        // validate "isIPv6"
        // validate "isAlpha"
        if (attribute.validate.isAlpha) {
          if (attribute.validate.isAlpha.msg) {
            template += ' CONSTRAINT "<%= constraintName %>"';
            replacements.constraintName = attribute.validate.isAlpha.msg;
          }
          template += ' CHECK ("<%= columnName %>" ~ <%= alpha %>)';
          replacements.columnName = attribute.fieldName;
          replacements.alpha = this.escape("^[a-zA-Z]+$", attribute);
        }
        
        // validate "isAlphanumeric"
        if (attribute.validate.isAlphanumeric) {
          if (attribute.validate.isAlphanumeric.msg) {
            template += ' CONSTRAINT "<%= constraintName %>"';
            replacements.constraintName = attribute.validate.isAlphanumeric.msg;
          }
          template += ' CHECK ("<%= columnName %>" ~ <%= alphaNumeric %>)';
          replacements.columnName = attribute.fieldName;
          replacements.alphaNumeric = this.escape("^[a-zA-Z0-9]+$", attribute);
        }

        // validate "isNumeric"
        if (attribute.validate.isNumeric) {
          if (attribute.validate.isNumeric.msg) {
            template += ' CONSTRAINT "<%= constraintName %>"';
            replacements.constraintName = attribute.validate.isNumeric.msg;
          }
          template += ' CHECK ("<%= columnName %>" ~ <%= numeric %>)';
          replacements.columnName = attribute.fieldName;
          replacements.numeric = this.escape("^-?[0-9]+(\.[0-9]+)?$", attribute);
        }

        // validate "isLowercase"
        if (attribute.validate.isLowercase) {
          if (attribute.validate.isLowercase.msg) {
            template += ' CONSTRAINT "<%= constraintName %>"';
            replacements.constraintName = attribute.validate.isLowercase.msg;
          }
          template += ' CHECK ("<%= columnName %>" = LOWER("<%= columnName %>"))';
          replacements.columnName = attribute.fieldName;
        }

        // validate "isUppercase"
        if (attribute.validate.isUppercase) {
          if (attribute.validate.isUppercase.msg) {
            template += ' CONSTRAINT "<%= constraintName %>"';
            replacements.constraintName = attribute.validate.isUppercase.msg;
          }
          template += ' CHECK ("<%= columnName %>" = UPPER("<%= columnName %>"))';
          replacements.columnName = attribute.fieldName;
        }

        // validate "notNull"
        if (attribute.validate.notNull) {
          if (attribute.validate.notNull.msg) {
            template += ' CONSTRAINT "<%= constraintName %>"';
            replacements.constraintName = attribute.validate.notNull.msg;
          }
          template += ' NOT NULL';
        }

        // validate "isNull"
        if (attribute.validate.isNull) {
          if (attribute.validate.isNull.msg) {
            template += ' CONSTRAINT "<%= constraintName %>"';
            replacements.constraintName = attribute.validate.isNull.msg;
          }
          template += ' CHECK ("<%= columnName %>" IS NULL)';
          replacements.columnName = attribute.fieldName;
        }

        // validate "notEmpty"
        if (attribute.validate.notEmpty) {
          if (attribute.validate.notEmpty.msg) {
            template += ' CONSTRAINT "<%= constraintName %>"';
            replacements.constraintName = attribute.validate.notEmpty.msg;
          }
          template += ' CHECK ("<%= columnName %>" != <%= emptyString %>)';
          replacements.columnName = attribute.fieldName;
          replacements.emptyString = this.escape("", attribute);
        }

        // validate "equals"
        if (attribute.validate.equals) {
          if (attribute.validate.equals.msg) {
            template += ' CONSTRAINT "<%= constraintName %>"';
            replacements.constraintName = attribute.validate.equals.msg;
          }
          template += ' CHECK ("<%= columnName %>" = <%= specificValue %>)';
          replacements.columnName = attribute.fieldName;
          replacements.specificValue = this.escape(attribute.validate.equals, attribute);
        }

        // validate "contains"
        if (attribute.validate.contains) {
          if (attribute.validate.contains.msg) {
            template += ' CONSTRAINT "<%= constraintName %>"';
            replacements.constraintName = attribute.validate.contains.msg;
            validator = attribute.validate.contains.args;
          }
          else {
            validator = attribute.validate.contains;
          }
          template += ' CHECK ("<%= columnName %>" ~ <%= substring %>)';
          replacements.columnName = attribute.fieldName;
          replacements.substring = this.escape(validator, attribute);
        }

        // validate "notContains" 
        if (attribute.validate.notContains) {
          if (attribute.validate.notContains.msg) {
            template += ' CONSTRAINT "<%= constraintName %>"';
            replacements.constraintName = attribute.validate.notContains.msg;
            validator = attribute.validate.notContains.args;
          }
          else {
            validator = attribute.validate.notContains;
          }
          template += ' CHECK ("<%= columnName %>" !~ <%= substring %>)';
          replacements.columnName = attribute.fieldName;
          replacements.substring = this.escape(validator, attribute);
        ...
        if (attribute.validate.len) {
          if (attribute.validate.len.msg) {
            template += ' CONSTRAINT "<%= constraintName %>"';
            replacements.constraintName = attribute.validate.len.msg;
            validator = attribute.validate.len.args;
          }
          else {
            validator = attribute.validate.len;
          }
          template += ' CHECK (LENGTH("<%= columnName %>") BETWEEN <%= minValue %> AND <%= maxValue %>)';
          replacements.columnName = attribute.fieldName;
          replacements.minValue = validator[0];
          replacements.maxValue = validator[1];
        }
